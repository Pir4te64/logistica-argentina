...
const useEstadoRepartidor = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(API_URL.ESTADO_REPARTIDOR, {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(API_URL.ESTADO_REPARTIDOR, formData, {
        headers: {
...
const handleEditSubmit = async (e) => {
  ...
  try {
    ...
    const response = await axios.put(
      `${API_URL.ESTADO_REPARTIDOR}/${editId}`,
      editFormData,
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${API_URL.ESTADO_REPARTIDOR}/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
...
export default useEstadoRepartidor;
