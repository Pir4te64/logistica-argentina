...
const Postulaciones = () => {
    ...
    const handleChange = (e) => {
        ...
        setForm(f => ({ ...f, [name]: name === 'puntos' ? parseInt(value) : (value === 'true' ? true : value === 'false' ? false : value) }));
    };
    ...
    if (loading) {
        return (
            <div className="container mx-auto px-4 py-8">
                <h1 className="text-3xl font-extrabold text-gray-800 mb-6">Postulaciones</h1>
                ...
                <table className="min-w-full divide-y divide-gray-200 bg-white shadow rounded-lg">
                    <thead className="bg-gray-50 sticky top-0">
                        <tr>
                            ...
                            <th className="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Cumple Req.</th>
                            <th className="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Asignado</th>
                            <th className="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Puntos</th>
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {postulaciones.map(p => (
                            <tr key={p.id} className="hover:bg-gray-100 transition-colors">
                                <td className="px-4 py-2 text-sm text-gray-700 break-words">{p.email}</td>
                                <td className="px-4 py-2 text-sm text-gray-700">{p.fecha_inicio_servicio.slice(0, 10)}</td>
    ...
    i < p.puntos ? (
      ...
      <FaRegStar key={i} className="inline-block w-4 h-4 text-gray-300" />
            ...
            {editing && (
                ...
                >
                    <h2 className="text-2xl font-semibold mb-4">Editar Postulación #{editing.id}</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Cumple Requisitos</label>
                            <select
